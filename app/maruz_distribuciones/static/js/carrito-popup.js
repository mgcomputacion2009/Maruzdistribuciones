/**
 * 🛒 CARRITO POPUP - MARUZ DISTRIBUCIONES
 * Sistema de popup completamente separado para el carrito
 */
...
class CarritoPopup {
    ...
    async init() {
        try {
            console.log('🛒 Inicializando carrito popup...');
            ...
            // 3. Ensure - Crear/obtener pedido único
            await this.ensurePedido();
            
            // 4. Conectar SSE para tiempo real
            ...
            console.log('✅ Carrito popup inicializado correctamente');
            
        } catch (error) {
            console.error('❌ Error inicializando carrito popup:', error);
        }
    }
    ...
    /**
     * 🔐 ENSURE - Crear/obtener pedido único
     */
    async ensurePedido() {
        try {
    ...
    connectSSE() {
        try {
            ...
            this.eventSource.onopen = () => {
                console.log('✅ SSE conectado para popup');
            ...
            this.eventSource.onerror = () => {
                console.warn('⚠️ Error SSE popup, activando fallback');
                this.activateFallbackPoll();
            };
            
        } catch (error) {
            console.error('❌ Error conectando SSE popup:', error);
            this.activateFallbackPoll();
        }
    }
    ...
    handleSSEMessage(event) {
        try {
            ...
            console.error('❌ Error procesando SSE popup:', error);
        }
    }
    ...
    handleItemUpdate(data) {
        const item = this.items.find(i => i.producto_codigo === data.producto_codigo);
        if (item) {
    ...
    /**
     * 🗑️ MANEJAR REMOCIÓN DE ITEM
     */
    handleItemRemove(data) {
        this.items = this.items.filter(i => i.producto_codigo !== data.producto_codigo);
        this.renderItems();
    }
    ...
    async flushChanges() {
        ...
        try {
            ...
            console.error('❌ Error en flush popup:', error);
        }
    }
    ...
    destroy() {
        ...
        console.log('🧹 Carrito popup destruido');
    }
}

// Inicializar cuando DOM esté listo
document.addEventListener('DOMContentLoaded', () => {
    window.carritoPopup = new CarritoPopup();
});

console.log('🛒 Carrito Popup cargado - Maruz Distribuciones');
